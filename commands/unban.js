...
module.exports = {
  name: 'unban',
  description: 'Débannir un utilisateur (admin only)',
  async execute(bot, msg, args) {
    ...
    if (args[0]) {
      ...
      return bot.sendMessage(msg.chat.id, 'Usage : /unban <id> ou reply à un message.');
    }
    ...
    if (!user.banned) return bot.sendMessage(msg.chat.id, 'Cet utilisateur n’est pas banni.');

    user.banned = false;
    updateUser(user);

    bot.sendMessage(msg.chat.id, `✅ L'utilisateur ${user.username || user.first_name} a été débanni.`);
    try {
